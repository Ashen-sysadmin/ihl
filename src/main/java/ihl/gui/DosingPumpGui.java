...
public class DosingPumpGui extends GuiContainer {
	...
	public DosingPumpGui(DosingPumpContainer container1) {
		...
		setpointTextField.setText(Integer.toString(this.container.getTileEntity().fluidAmountSetpoint));
		setpointTextField.setFocused(true);
	}

	@Override
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		if(lastFluidAmountSetpoint!= this.container.getTileEntity().fluidAmountSetpoint){
			setpointTextField.setText(Integer.toString(this.container.getTileEntity().fluidAmountSetpoint));
			lastFluidAmountSetpoint= this.container.getTileEntity().fluidAmountSetpoint;
		}
		...
		i1 = TANK_HEIGHT - this.container.getTileEntity().fluidAmountSetpoint * TANK_HEIGHT
				/ this.container.getTileEntity().getFluidTank().getCapacity();
		this.drawTexturedModalRect(78, 6 + i1, 176, 14, 25, 7);
		if (this.container.getTileEntity().getTankAmount() > 0) {
			IHLRenderUtils.instance.renderIHLFluidTank(this.container.getTileEntity().getFluidTank(), 82, 10, 94,
	...
	protected void keyTyped(char characterTyped, int keyIndex) {
		...
		if (keyIndex == KeyEvent.VK_ACCEPT || keyIndex == KeyEvent.VK_ENTER || keyIndex == 28 || keyIndex == 156) {
			int fluidAmountSetpoint = (short) Math.max(1,
					Math.min(this.container.getTileEntity().getFluidTank().getCapacity(),
			...
			((ClientProxy)IHLMod.proxy).sendIntegerFieldValueFromClientToServer(fluidAmountSetpoint, "fluidAmountSetpoint", this.container.getTileEntity());
		}
	}
	@Override
