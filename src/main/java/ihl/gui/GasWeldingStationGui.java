...
import ihl.container.GasWeldingStationContainer;
...
public class GasWeldingStationGui extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        int yOffset = (this.height - ySize) / 2;
            ...
            int i1;
            if (this.container.getTileEntity().progress2 > 0)
            {
            	i1 = Math.min(this.container.getTileEntity().gaugeProgressScaled(27),27);
            	this.drawTexturedModalRect(90, 30, getFrameX(i1), getFrameY(i1),24,24);
            }
            if (this.container.getTileEntity().flammableGasTank.getFluid()!=null && this.container.getTileEntity().flammableGasTank.getFluidAmount() > 0)
            {
            	IHLRenderUtils.instance.renderIHLFluidTank(this.container.getTileEntity().flammableGasTank, 64, 28, 76, 67, zLevel, par1, par2, xOffset, yOffset);
            }
            if (this.container.getTileEntity().oxygenTank.getFluid()!=null && this.container.getTileEntity().oxygenTank.getFluidAmount() > 0)
            {
            	IHLRenderUtils.instance.renderIHLFluidTank(this.container.getTileEntity().oxygenTank, 28, 28, 40, 67, zLevel, par1, par2, xOffset, yOffset);
            }
    ...
    public void handleMouseClick(Slot slot,int arg1,int arg2,int arg3)
    {
    	...
    	{
    		IC2.network.get().initiateClientTileEntityEvent(this.container.getTileEntity(), slot.slotNumber-outputslotoffset+1);
    	}
